...
int XAudioEngin::load(const std::string &audio,
    ...
    if (handelit != handles.end()) {
        ...
        if (avformat_open_input(&format, path.string().c_str(), nullptr,
            ...
            const char *title =
                av_dict_get(format->metadata, "title", NULL, 0)
                    ? av_dict_get(format->metadata, "title", NULL, 0)->value
                    : "N/A";
            const char *artist =
                av_dict_get(format->metadata, "artist", NULL, 0)
                    ? av_dict_get(format->metadata, "artist", NULL, 0)->value
                    : "N/A";
            sound->artist = std::string(artist);
            sound->title = std::string(title);

            XINFO("artist:" + sound->artist + "->title:" + sound->title);
...
void XAudioEngin::unload(int id) {
    ...
    if (audioit != audios.end()) {
        ...
        XINFO("已卸载[" + audioit->second->name + "],句柄:[" +
              std::to_string(id) + "]");
        // 删除音频
        audios.erase(audioit);
    } else {
...
// 获取音频元数据
std::string XAudioEngin::get_audio_titleunicode(const std::string &auido) {
    auto id_it = handles.find(auido);
    if (id_it == handles.end()) return "N/A";
    std::shared_ptr<XSound> res;
    is_audio_loaded(id_it->second, res);
    return res->title;
}
std::string XAudioEngin::get_audio_artistunicode(const std::string &auido) {
    auto id_it = handles.find(auido);
    if (id_it == handles.end()) return "N/A";
    std::shared_ptr<XSound> res;
    is_audio_loaded(id_it->second, res);
...
float XAudioEngin::volume(int deviceid, int id) {
    ...
    if (!player) {
        XWARN("设备[" + std::to_string(deviceid) + ":" +
...
void XAudioEngin::pos(int deviceid, int id, int64_t time) {
    ...
    if (orbitsit == mixer->audio_orbits.end()) {
        XWARN("设备[" + std::to_string(deviceid) + ":" +
              outdevice->device_name + "]上不存在音源[" + std::to_string(id) +
              "]");
        return;
    }
...
void XAudioEngin::play(int device_index, int audio_id, bool loop) {
    ...
    if (orbitsit == mixer->audio_orbits.end()) {
        XWARN("[" + std::to_string(device_index) + ":" +
              outdevice->device_name + "]设备音轨中不存在音源[" +
        ...
        XINFO("已添加播放音频句柄[" + std::to_string(audio_id) + ":" +
              audio->name + "]到[" + std::to_string(device_index) + ":" +
              outdevice->device_name + "]音轨");
    } else {
        for (const auto &orbit : orbitsit->second) {
            if (orbit->paused) {
                // 存在
                XWARN("检测到音频暂停,继续播放句柄[" +
                      std::to_string(audio_id) + "]");
                // 更新音频暂停标识
                orbit->paused = false;
            } else {
...
bool XAudioEngin::is_pause(int device_id) {
    ...
    const auto &player = outdevice->player;
    if (!player) {
        XWARN("设备[" + std::to_string(device_id) + ":" +
              outdevice->device_name + "]还没有初始化播放器");
        return false;
    }
