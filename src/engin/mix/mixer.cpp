...
void XAuidoMixer::add_orbit_immediatly(
    ...
    std::lock_guard<std::mutex> lock(immediate_mtx);
    orbit->loop = false;
    ...
    for (int i = 0; i < src_sounds.size(); i++) {
        ...
        if (speed == 1.0) {
            for (int k = 0; k < static_cast<int>(x::Config::channel); k++) {
                for (int j = 0; j < actrual_play_data_size; j++) {
                    if (playpos + j < (double)audio->sound->pcm[k].size()) {
                        // 第i个音频第k个声道第j个数据
                        pcms[i][k][j] =
                            audio->sound->pcm[k][(int)(playpos + j)] *
                            audio->volume;
                    } else {
                        // 超过结尾填充0
                        pcms[i][k][j] = 0;
            ...
            if (speed < 1.0) {
                ...
                // 实际取样开始位置
                auto startpos =
                    playpos - (des_size - actrual_play_data_size) / 2.0;
                ...
                for (int k = 0; k < static_cast<int>(x::Config::channel); k++) {
                    for (int j = 0; j < des_size; j++) {
                        if (startpos + j <
                                (double)audio->sound->pcm[k].size() &&
                            startpos + j > 0) {
                            // 第i个音频第k个声道第j个数据
                            temp_pcms[k][j] =
                                audio->sound->pcm[k][(int)(startpos + j)] *
                                audio->volume;
                        } else {
                            // 超过结尾或开头填充0
                            temp_pcms[k][j] = 0;
                ...
                // + "]");
                auto pos_in_sample_pcm =
                    (des_size - actrual_play_data_size) / 2.0;
                // XWARN("播放位置(在拉伸前的样本中)[" +
                ...
                for (int k = 0; k < static_cast<int>(x::Config::channel); k++) {
                    for (int j = 0; j < des_size; j++) {
                        // 第i个音频第k个声道第j个数据
                        pcms[i][k][j] =
                            temp_pcms[k][(int)(pos_in_temppcm + j)] *
                            audio->volume;
                    }
                }
            } else {
